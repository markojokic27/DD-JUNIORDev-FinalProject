...
function SelectOneInput(props) {
  ...
  const handleOptionClick = (optionId) => {
    const isSelected = props.data[props.name] === optionId;
    if (isSelected) {
      props.setData({ ...props.data, [props.name]: null }); 
      props.setIsValid({ ...props.isValid, [props.name]: false }); 
    } else {
      props.setData({ ...props.data, [props.name]: optionId }); 
          ...
          {props.options.map((option) => (
            <div key={option.id} className={classes.option} onClick={() => handleOptionClick(option.id)}>
              <input
                type="checkbox"
                id={option.id}
                checked={props.data[props.name] === option.id}
                readOnly
              />
